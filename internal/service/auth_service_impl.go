...
func (s *authServiceImpl) RefreshToken(ctx context.Context, userId, sessionId, role string) (*dto.RefreshTokenResponse, error) {
...
func (s *authServiceImpl) ResetPassword(ctx context.Context, token string, newPassword string) error {
	...
	session, err := s.sessionRepo.GetByToken(ctx, hashedToken)
	if err != nil {
